...
const { container, virtualDir } = getAzureDeployConfig();
...
if (dryRun) {
  console.log(`${appName} deploy dry run complete. No blobs were changed.`);
} else {
  console.log(
    `${appName} deployed to https://${storageAccount}.blob.core.windows.net/${container}/${virtualDir}/`,
  );
}
